...
def get_historial(id_user):
    cur = mysql.connection.cursor()
    cur.execute('SELECT cliente.apellido, cliente.nombre, cliente.cuit, factura_productos.CANTIDAD, factura_productos.PRECIO_PRODUCTO, producto.nombre_producto, factura.fecha_factura, factura.ID, factura.id_usuario FROM factura JOIN cliente ON cliente.id = factura.ID_CLIENTE JOIN factura_productos ON factura_productos.ID_FACTURA = factura.ID JOIN producto ON factura_productos.ID_PRODUCTO = producto.id JOIN usuario ON factura.ID_USUARIO = usuario.id WHERE factura.ID_USUARIO = %s GROUP BY factura.fecha_factura, factura.id_usuario, cliente.id, cliente.id_usuario, cliente.nombre, cliente.apellido, cliente.cuit, factura_productos.ID_FACTURA, factura_productos.ID_PRODUCTO, factura_productos.CANTIDAD, producto.nombre_producto, producto.precio, usuario.nombre, usuario.id;',(id_user,))
    data = cur.fetchall()
    historialList = []
    for row in data:
...
@app.route('/user/<int:id_user>/cliente/<int:id_client>', methods = ['DELETE'])
@token_required
@user_resources
@client_resource
